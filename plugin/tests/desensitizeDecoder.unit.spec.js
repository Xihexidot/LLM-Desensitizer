...
test.describe("decodeText 解码还原", () => {
  test("精准还原 [PHONE_1]、[MASKED_1] 等脱敏标记为原始数据", () => {
    ...
    const text = "您的联系方式是[PHONE_1]，证件号[MASKED_1]，请核对。";
...
test.describe("buildMaskMapping 兜底映射重建", () => {
  test("按类型与序号建立映射（后端序号按 start 降序分配）", () => {
    const mapping = decoder.buildMaskMapping("电话[PHONE_1]与[PHONE_2]", [
  ...
  test("同类型多类型混合映射", () => {
    const mapping = decoder.buildMaskMapping(
      ...
      [
        {
          type: "PHONE_NUMBER",
          originalText: "13800138000",
          start: 3,
          end: 14,
        },
        {
          type: "ID_CARD",
          originalText: "510104198303123639",
          start: 18,
          end: 36,
        },
      ],
  ...
  test("同一明文多次出现时复用同一占位符", () => {
    const mapping = decoder.buildMaskMapping("名字[NAME_1]和[NAME_1]", [
